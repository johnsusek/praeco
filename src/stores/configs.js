  ...
  actions: {
    ...
    async moveConfig({ oldConfig, newPath, type }) {
      ...
      if (type === 'rules') {
        ...
        const baseRuleConfigExists = await this.checkBaseRuleConfigExists();
    ...
    async disableRule(conf) {
      const { useConfigStore } = await import('./config/index.js');
      ...
      try {
        ...
        if (res.data.created) {
          configStore.settings.updateEnabled(false);
          this.updatedConfig({
    ...
    async enableRule(conf) {
      ...
      try {
        ...
        if (res.data.created) {
          configStore.settings.updateEnabled(true);
          this.updatedConfig({
            path: conf.__praeco_full_path,
            type: 'rules',
            config: conf
